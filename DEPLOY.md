...
### Running in GitHub Codespaces with Expo Go (iPhone)

If you see `Cannot determine the project's Expo SDK version because the module 'expo' is not installed`, dependencies were not installed in the current Codespace.
...
pnpm install
cp .env.example .env
# optional: fill values needed by your backend flows

# Start Expo in tunnel mode for iPhone over Expo Go
pnpm expo:go
```

If Expo fails in restricted/proxied environments with `TypeError: fetch failed`, retry in offline mode:

```bash
pnpm expo:go:offline
```

Notes:
- `pnpm dev` starts backend + Expo for local development.
- `pnpm dev:metro` starts native Expo Metro (QR for Expo Go).
